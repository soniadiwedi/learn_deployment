...
let outputArray=inputArray.map((el,index)=>{
  let obj={};
  obj['productId']=el.idMeal;
  obj['productTitle']=el.strMeal;
  obj['Category']=categoriesDrectory[el.Category];
  obj['Area']=areasDirectory[el.Area];
  obj['Ingredients']=[];

  for(let i=1;i<=20;i++){
    let obj1={};
    if(el[`strIngredient${i}`]!=null && el[`strIngredient${i}`]!="" ){
        obj1['ingredient']=el[`strIngredient${i}`];
        obj1['measure']=el[`strMeasure${i}`];
        obj['Ingredients'].push(obj1);
    }
  }
  return obj;
